...
# group by Source ID and check if all Leveraged Flag values are 'Y'
always_y = df.groupby('Source ID')['Leveraged Flag'].apply(lambda x: (x == 'Y').all())

# create new column 'Always Y' with True values where Leveraged Flag is always 'Y'
df['Always Y'] = df['Source ID'].map(always_y)
